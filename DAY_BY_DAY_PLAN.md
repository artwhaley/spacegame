...
Use the human character assets already available in the project (including the imported
content under `Assets/PolygonSciFiWorlds`). Keep the simulation object and its existing
state intact; replace or attach the **visual body**, not the identity of the colonist.
...
- Locate and read the existing prototype before designing a replacement. In this local
  repository the extracted source is `Packages/com.asteroidcolony.interactions`; if a
  newer external source is supplied, compare it rather than reconstructing the system
  from this plan.
