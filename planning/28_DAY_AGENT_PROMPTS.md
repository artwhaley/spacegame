...
Goal: port the minimum reusable pieces needed for one Farm worker to perform a visible local work cycle after the simulation already considers that worker active at the Farm. In this repository, inspect `Packages/com.asteroidcolony.interactions` first; it is the extracted prototype source.
